...
class shenpCog(commands.Cog):
    def __init__(self, client):
        ...
        self.colour = nextcord.Color.red #discord.Colour.from_rgb(*client.config['command_colour']['cog_shenp'])
        self.rel_path = ut.full_path(self.client.dir, self.client.config['configs']['shen'])
        self.post_path = ut.full_path(self.rel_path, "other")
    ...
    async def make_shenp(self, channel, users:tuple, paths:tuple, save_name:str, ames=False):
        # load bg
        open_files = []
        bg = Image.open(ut.full_path(self.rel_path, paths[0])) #os.path.join(self.client.dir, self.client.config['shen_path'], paths[0]))
        ...
        for user_dict in users:
            try:
                if ames:
                    if await self.ames_check(user_dict['user'], channel):
                        bg.close()
                        return False
                temp = requests.get(user_dict['user'].display_avatar.url)
            ...
            temp.resize(user_dict['size'],resample=Image.Resampling.BICUBIC)#Image.ANTIALIAS)
        ...
        for other_parts in paths[1:]:
            temp = Image.open(ut.full_path(self.rel_path, other_parts))
            bg.paste(temp, (0,0), temp)
            open_files.append(temp)
        
        bg.save(ut.full_path(self.post_path, save_name))
        [i.close() for i in open_files]

        return nextcord.File(ut.full_path(self.post_path, save_name)), save_name
    ...
    async def spray(self, ctx, user:str=None):
        ...
        async with ctx.typing():
            ...
            if user == False: 
                ...
                await channel.send(file=nextcord.File(ut.full_path(self.rel_path,"other","spray.png")))
                return
            user = {
                "user":user,
                "size":(150,150),
    ...
    async def muimi(self, ctx, url:str=None):
        ...
        async with ctx.typing():
            ...
            if out:
                ...
                await channel.send(file=nextcord.File(out))
    ...
    async def muimi_embed(self, ctx, url:str):
        ...
        # load fg
        muimi_fg = Image.open(ut.full_path(self.rel_path, "other", "muimi_this_c.png"))
        ...
        bg = bg.resize((final_width, target_dim[1]), resample=Image.Resampling.BICUBIC)
    ...
    async def wide(self, ctx, *request):
        ...
        async with ctx.typing():
            ...
            multi = 5
            blob = Image.open(ut.full_path(self.rel_path, "other","hatsuneblob.png"))
            ...
            if not animated:
                path = ut.full_path(self.post_path, "blob.png")
                blob = blob.resize(target, resample=Image.Resampling.BICUBIC)#Image.ANTIALIAS)
                blob.save(path)
                blob.close()
            else:
                blob.seek(0)
                duration = blob.info['duration']
                path = ut.full_path(self.post_path, "blob.gif")
                _blob = [frame.resize(target, resample=Image.Resampling.BICUBIC) for frame in ImageSequence.Iterator(blob)]
                _blob[0].save(
                    path,
                    format='GIF',
            ...
            blob.close()
            await channel.send(file=nextcord.File(path))
