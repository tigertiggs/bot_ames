...
class shenCog(commands.Cog):
    def __init__(self, client):
        ...
        self.colour = nextcord.Color.blue

        self.rel_path = ut.full_path(self.client.dir, self.client.config['configs']['shen'])

        with open(ut.full_path(self.rel_path,"other","_config.json"), "r") as c:
            self.config = json.load(c)
        
        if not self.client.config['imgur']['token']:
            self.imgur = Imgur({"client_id":self.client.config['imgur']['id'], "client_secret": self.client.config['imgur']['secret']})
            self.client.config['imgur']['token'] = self.imgur.authorize()
            print(self.name, "access token expired or missing for imgur")
            return
        else:
            self.imgur = Imgur({"client_id":self.client.config['imgur']['id'], "client_secret": self.client.config['imgur']['secret'], "access_token": self.client.config['imgur']['token']})
    ...
    async def shen(self, ctx, *, cmd):
        ...
        if ctx.invoked_subcommand is None and cmd in self.config: #and cmd in self.config and self.client.check_perm(author):
            #if self.client.check_perm(author) or author.id in self.config[cmd]['mods']:
            #    pass
            #else:
            #    pass
            ...
            if not (self.client.check_perm(author) or author.id in active['mods']):
                await channel.send(embed=embed)
                return

            pages = [embed]
            column_max = 30
            chunks = list(ut.chunks(active['images'], column_max))
            n_pages = len(chunks)
    ...
    def make_shen_simple(self, name, active, mods):
        ...
        embed.set_footer(text="Roulette | Re:Re:Write Ames",icon_url=self.client.user.display_avatar.url)
    ...
    def make_shen_list(self, name, i, column_max, n_pages, chunk):
        embed = nextcord.Embed(
                timestamp=datetime.datetime.utcnow())#, colour=self.colour)
        embed.set_footer(text="Roulette List | Re:Re:Write Ames",icon_url=self.client.user.display_avatar.url)
        embed.set_author(name=f"[{name}] Pool Index ({i+1}/{n_pages})")

        for j, column in enumerate(ut.chunks(chunk,column_max//3)):
            embed.add_field(
                name="Entries",
    ...
    async def add(self, ctx, *, cmd):
        ...
        #check perms
        if self.client.check_perm(author):
            pass
    ...
    async def big(self, ctx, emote:str="",num:int=1):
        ...
        if emote1[0] == '' and len(emote1) > 1:
            ...
            targets = list(filter(lambda x: not x.guild_id in (self.client.config['resource_servers'] + self.client.config['bo_resource_servers']) and sm(None, emote.lower(), x.name.lower(), None).ratio() >= 0.25 and emote.lower() in x.name.lower(), self.client.emojis))
            if len(targets) > 0:
        ...
        embed.set_author(name=f"{author.name} sent:",icon_url=author.display_avatar.url)
        #print(link)
    ...
    async def find(self, ctx, option:str=None):
        ...
        emotes = list(filter(lambda x: not x.guild_id in (self.client.config["resource_servers"] + self.client.config['bo_resource_servers']), self.client.emojis))
        ...
        if option == None:
            ...
            embed = nextcord.Embed(description=f'Listing the top {max} results correspoinding to `{option}`',
                                    timestamp=datetime.datetime.utcnow())#,colour=self.colour)
            embed.set_footer(text='Find Page | Re:Re:Write Ames',icon_url=self.client.user.display_avatar.url)
    ...
    def make_find_embed(self, emotes, page):
        embed = nextcord.Embed(timestamp=datetime.datetime.utcnow())#, colour=self.colour)
        embed.set_footer(text='Find Page | Re:Re:Write Ames',icon_url=self.client.user.display_avatar.url)
        embed.set_author(name=f"Emotedex - Page {page[0]} of {page[1]}")

        for column in ut.chunks(emotes, 20):
            embed.add_field(
                name='Emotes',
