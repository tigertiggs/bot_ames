            ...
            try:
                ...
                if ctx.message.guild: # textchannel/DMchannel
                    if str(ctx.message.guild.id) in self.blueoath_config['restricted_servers'] and ctx.command:
                        if not ctx.command.cog.qualified_name in self.blueoath_config['ames_allowed_cogs'] and not self.config['debug'] and not ctx.message.channel.id in self.blueoath_config['restricted_servers'][str(ctx.message.guild.id)]['exceptions']:
